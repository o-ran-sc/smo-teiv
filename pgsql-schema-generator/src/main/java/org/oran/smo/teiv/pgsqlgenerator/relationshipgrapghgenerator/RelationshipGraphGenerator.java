    ...
    public void generate(List<Relationship> relationships) {
        if (generateRelationshipGraph) {
            ...
            for (String moduleName : moduleNames) {
                ...
                if (moduleRelationships.isEmpty()) {
                    log.info("No Relationships found in the {} model", moduleName);
                } else {
                    generateGraph(moduleName, moduleRelationships);
                }
            }
            generateGraph("overall", relationships);
        } else {
            log.info("generate-relationship-graph set to false");
        }
    }

    private void generateGraph(String name, List<Relationship> relationships) {
        MutableGraph g = prepareGraph(relationships);
        File outputFile = new File(graphOutput, name);
        try {
            Graphviz.fromGraph(g).render(Format.SVG).toFile(outputFile);
        } catch (IOException e) {
            log.error("Error when rendering {} graph to svg", name);
            throw new RuntimeException(e);
        }
        System.out.println("Graph rendered to: " + outputFile.getAbsolutePath());
    }

    private MutableGraph prepareGraph(List<Relationship> moduleRelationships) {
        ...
        for (Relationship moduleRelationship : moduleRelationships) {
            ...
            MutableNode sourceNode = Factory.mutNode(moduleRelationship.getASideMOType());
            MutableNode targetNode = Factory.mutNode(moduleRelationship.getBSideMOType());
            g.add(sourceNode.addLink(Factory.to(targetNode).with(Label.of(label))));
        }
        return g;
    }
}
